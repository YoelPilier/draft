...
the template shall meet the \oldconcept{UnaryTypeTrait} requirements\iref{meta.rqmts}
with a base characteristic of
\begin{codeblock}
integral_constant<size_t, TS::value>
\end{codeblock}
Otherwise, it shall have no member \tcode{value}.
...
the template shall meet the \oldconcept{TransformationTrait}
requirements\iref{meta.rqmts} with a member typedef \tcode{type} that names the
type \tcode{add_const_t<TE>}.
...
type \tcode{T}. Then the template shall meet the
\oldconcept{UnaryTypeTrait} requirements\iref{meta.rqmts} with a
...
cv-unqualified type \tcode{T}. Then the template shall
