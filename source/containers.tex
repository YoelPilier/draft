...
\pnum
\effects
Equivalent to: \tcode{return span<element_type, Count>\{data(), Count\};}
\end{itemdescr}
...
Equivalent to:
\begin{codeblock}
auto p = reinterpret_cast<byte*>(s.data());
auto n = s.size_bytes();
if constexpr (Extent == dynamic_extent)
  return span<byte>{p, n};
else
  return span<byte, sizeof(ElementType) * Extent>{p, n};
\end{codeblock}
\end{itemdescr}
