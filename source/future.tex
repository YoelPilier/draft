...
the \tcode{TransformationTrait} requirements with a base characteristic of
\begin{codeblock}
...
Then each of the two templates shall satisfy
the \tcode{TransformationTrait} requirements
with a member typedef type that names the following type:
\begin{itemize}
\item for the first specialization, add_volatile_t<TE>, and
\item for the second specialization, add_cv_t<TE>.
\end{itemize}
...
of the cv-unqualified type \tcode{T}.
Then each of the two templates shall meet
the \tcode{TransformationTrait} requirements
with a member typedef type that names the following type:
\begin{itemize}
...
then that overload is available when \tcode{atomic<T>::is_always_lock_free}
is \tcode{false}:
\begin{codeblock}
