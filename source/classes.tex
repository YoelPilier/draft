...
\indextext{function!member!special!defaulted!constexpr-compatible}%
\defn{constexpr-compatible}
...
\indextext{operator!\idxcode{<=>}!deleted}%
\indextext{operator!\idxcode{==}!deleted}%
...
\defnx{usable}{usable!\idxcode{operator@}}
if either
...
\indextext{operator!comparison!defaulted!constexpr-compatible}%
...
no overload resolution was performed
when determining whether to delete the function results
in a usable candidate that is a non-constexpr function.
\begin{note}
...
if $\tcode{R}_i$ is not
a comparison category type\iref{cmp.categories.pre} for any \tcode{i}.
